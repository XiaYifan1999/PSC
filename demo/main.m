...
binary_weight = 0.009;
unmatch_cost = 0.8;
[match_dist, ~, init_seed_matches, prog_matches] = ProgressiveFeatureMatching (...
    single(tar_feat), single(ref_feat), single(tar_desc), single(ref_desc), ...
    single(binary_weight), single(unmatch_cost) );
f_matches = prog_matches+1;  % change to one base.
toc;
figure;
image(newfig);
axis image; axis off;
...
if gt
    ...
    pfm = sprintf('#matches: %d, #inliers: %d, PC: %.4f', size(f_matches,2), sum(inliers), sum(inliers)/size(f_matches, 2))
    text(t_xd, t_yd, pfm, 'FontUnits', 'pixels', 'FontSize', 10, 'Color', [0.95,0.95,0.95], 'BackgroundColor', [0.2,0.2,0.2]);  
else
    line([f1(1,f_matches(1,pind));f2(1,f_matches(2,pind))],[f1(2,f_matches(1,pind));f2(2,f_matches(2,pind))],'linewidth',1,'color','m');
    hold off;
end
